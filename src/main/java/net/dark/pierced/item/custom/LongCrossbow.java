package net.dark.pierced.item.custom;
...
public class LongCrossbow extends CrossbowItem {
    ...
    protected void shoot(LivingEntity shooter, ProjectileEntity projectile, int index, float speed, float divergence, float yaw, @Nullable LivingEntity target) {
        ...
        projectile.setVelocity((double)vector3f.x(), (double)vector3f.y(), (double)vector3f.z(), speed, 0);
    ...
    public LongCrossbow(Settings settings) {
        super(settings);
    }
    ...
    public boolean canBeEnchantedWith(ItemStack stack, RegistryEntry<Enchantment> enchantment, EnchantingContext context) {
        if (enchantment.matchesKey(Enchantments.UNBREAKING)) {
            ...
            return true;
        } else if (enchantment.matchesKey(Enchantments.BINDING_CURSE)) {
            return true;
